...
void generate_truck(int truck_num, std::string map_name) {
    ...
    vehicle_truck->ApplyControl(cc::Vehicle::Control{
        0.0f,   // throttle
        0.0f,   // steer
        1.0f,   // brake (full)
        false,  // hand_brake
        false,  // reverse
        false,  // manual gear shift
        0    // hand_brake = false
    });
    vehicle_truck->SetTargetVelocity(carla::geom::Vector3D(0.0f, 0.0f, 0.0f));
    vehicle_truck->SetTargetAngularVelocity(carla::geom::Vector3D(0.0f, 0.0f, 0.0f));
