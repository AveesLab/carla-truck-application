...
void TruckController::server_enu_callback(const geometry_msgs::msg::Point::SharedPtr msg)
{
    ...
    //if(actor_id_==0) RCLCPP_INFO(this->get_logger(), "Stored starting position: (%.2f, %.2f)", cur_x_, cur_y_);
...
void TruckController::compute_control()
{
    ...
    if (current_state_ == ControllerState::RUNNING)
    {
        ...
        { // 모든 선분을 탐색했는데도 교점을 못 찾은 경우
            ...
            std::cout<<"can not find ld point"<<std::endl;
        ...
        else
        {
            ...
            //} 
              ...
              // formation_id_ != 0 블록 끝
        }  // lane change control else 블록 끝
        ...
        if(actor_id_ == 0)
        {
            ...
            if(stop_flag_==true) throttle_value = -1;
